...
void accionDespedir() {
    bocaAbrir();
    player.play(2);  // 002.mp3 → archivo 001 es index 1 en DFPlayer
    delay(3000);
    bocaCerrar();
}
...
void funcTemperatura() {
    ...
    if (t >= 15.0f && t <= 25.0f) { funcEnCurso = false; return; }
...
:root{
  --rosa:#FF2D78;--azul:#00F5FF;--negro:#050510;
  --rosa2:#FF69B4;--azul2:#0099CC;--gris:#1A1A2E;
}
*{box-sizing:border-box;margin:0;padding:0;}
body{background:var(--negro);color:var(--azul);font-family:'Press Start 2P',monospace;min-height:100vh;
  background-image:radial-gradient(circle at 20% 50%,rgba(255,45,120,0.08) 0%,transparent 50%),
  radial-gradient(circle at 80% 20%,rgba(0,245,255,0.08) 0%,transparent 50%);
}
header{text-align:center;padding:24px 16px 8px;border-bottom:2px solid var(--rosa);}
header h1{font-size:clamp(14px,4vw,28px);color:var(--rosa);text-shadow:0 0 12px var(--rosa),0 0 30px var(--rosa2);
  letter-spacing:4px;animation:pulso 2s ease-in-out infinite;}
header p{font-size:8px;color:var(--azul2);margin-top:8px;letter-spacing:2px;}
@keyframes pulso{0%,100%{opacity:1;}50%{opacity:.7;}}

.status{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:16px;max-width:600px;margin:0 auto;}
.stat-box{background:var(--gris);border:2px solid var(--azul2);padding:12px;border-radius:4px;}
.stat-box h3{font-size:7px;color:var(--rosa);margin-bottom:8px;letter-spacing:1px;}
.stat-val{font-family:'VT323',monospace;font-size:28px;color:var(--azul);}
.countdown{font-family:'VT323',monospace;font-size:22px;color:var(--rosa2);}
.label-sm{font-size:6px;color:#666;margin-top:4px;}

.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));
  gap:10px;padding:16px;max-width:700px;margin:0 auto;}

.btn{background:var(--negro);border:2px solid var(--azul);color:var(--azul);
  font-family:'Press Start 2P',monospace;font-size:7px;padding:12px 6px;cursor:pointer;
  border-radius:2px;text-align:center;line-height:1.7;transition:all .15s;
  text-shadow:0 0 6px var(--azul);position:relative;overflow:hidden;}
.btn:hover{background:var(--azul);color:var(--negro);text-shadow:none;
  box-shadow:0 0 18px var(--azul),0 0 40px rgba(0,245,255,.3);}
...
.btn.fiesta{border-color:#FFD700;color:#FFD700;font-size:8px;}
.btn.fiesta:hover{background:#FFD700;color:var(--negro);box-shadow:0 0 24px gold;}
.btn .ico{font-size:18px;display:block;margin-bottom:6px;}

.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);
  background:var(--gris);border:2px solid var(--rosa);color:var(--rosa);
...
footer{text-align:center;font-size:6px;color:#333;padding:24px;letter-spacing:2px;}
</style></head><body>
...
<div class="status">
  <div class="stat-box">
    ...
    <div class="stat-val" id="hum" style="font-size:20px;">-- %</div>
...
void handleCmd() {
    ...
    // Ejecutar en el loop siguiente para no bloquear HTTP
